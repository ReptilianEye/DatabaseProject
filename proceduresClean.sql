CREATE PROCEDURE AddEducationFormPaymentsDetails
    @educationFormId   int,
    @advanceDue        int,
    @advance           money,
    @wholePrice        money,
...
BEGIN
    ...
    begin
        throw 50000, 'Nie istnieje taka forma edukacji', 1;
    end

    if @wholePriceDue < getdate()
    begin
        throw 50000, 'Błędna data płatności', 1;
    end

    if @advance < 0 or @wholePrice < 0
    begin
        throw 50000, 'Niepoprawna cena', 1;
    end

    declare @priceId int = dbo.nextPriceId();
    ...
    DECLARE @advanceDueDate date;
    SET @advanceDueDate = DATEADD(DAY, -@advanceDue, @wholePriceDue);
...
CREATE PROCEDURE AddFinishedPractice
 @userId INT,
...
BEGIN
   ...
   SELECT @studiesId = studiesId FROM Studies WHERE name = @nazwaStudiow;


   IF @studiesId IS NOT NULL
   BEGIN
       IF (@dataRozpoczecia IS NOT NULL AND @dataZakonczenia IS NOT NULL) AND (@dataRozpoczecia <= @dataZakonczenia)
       BEGIN
           INSERT INTO Practices ([studiesId], [userId], [startDate], [endDate])
           VALUES (@studiesId, @userId, @dataRozpoczecia, @dataZakonczenia);
           PRINT 'Zakonczona praktyka dodana pomyslnie.';
       END
       ELSE
       BEGIN
           THROW 50000, 'Nieprawidłowa data', 1;
       END
   END
   ELSE
   BEGIN
       THROW 50000, 'Studia nie znalezione', 1;
   END
END
...
GRANT EXECUTE ON AddFinishedPractice TO practice_supervisor
GO
...
BEGIN
    ...
    BEGIN
        Throw 50000, 'Błąd: Podany użytkownik nie istnieje.', 1
    END

    -- Sprawdzenie, czy użytkownik istnieje
    IF NOT EXISTS (SELECT 1 FROM [Subjects] WHERE [title] = @SubjectTitle)
    BEGIN
        THROW 50000, 'Błąd: Podany przedmiot nie istnieje.', 1
    END

    -- Sprawdzenie, czy ocena istnieje
    IF NOT EXISTS (SELECT 1 FROM [Grades] WHERE [grade] = @Grade)
    BEGIN
        THROW 50000, 'Błąd: Podana ocena nie istnieje.', 1
    END

    IF NOT EXISTS (SELECT 1 FROM [Studies] WHERE [name] = @StudiesTitle)
    BEGIN
        THROW 50000, 'Błąd: Nie ma takiego kierunku.', 1
    END

    -- Dodanie oceny do tabeli StudentsGrades
    INSERT INTO [StudentsGrades] (userId, examId, gradeId)
    VALUES (@UserId, (select top 1 examId from Exams where studiesId = (select studiesId from Studies where name = @StudiesTitle) and
                                                           subjectId = (select subjectId from Subjects where title = @SubjectTitle)),
            (select gradeId from Grades where grade = @Grade));

END;
GO

create procedure AddNewSubjectToStudies
    @studiesName varchar(255),
    @subjectName varchar(255),
    @semester int
as
begin
    IF NOT EXISTS (SELECT 1 FROM [Studies] WHERE name = @studiesName)
    BEGIN
       THROW 50000, 'Podane studia nie istnieja', 1;
    END
    IF NOT EXISTS (SELECT 1 FROM [Subjects] WHERE [Subjects].title = @subjectName)
   BEGIN
       THROW 50000, 'Nie ma takiego przedmiotu.', 1;
   END
   IF @semester > 10
    begin
        THROW 50000, 'Nie ma semestrów powyżej 10', 1;
    end
    begin
        insert into Syllabuses (syllabusId, subjectId, semester)
        values ((select studiesId from Studies where name = @studiesName),
...
CREATE PROCEDURE AddNewTranslatedLanguage
   @languageID INT,
   @languageName VARCHAR (255)
AS
BEGIN
   IF NOT EXISTS (SELECT 1 FROM [LanguagesDetails] WHERE [languageId] = @languageID)
    BEGIN
       throw 50000, 'Błąd: Język o takim id jest juz w bazie danych.', 1
    END
...
begin
    ...
    begin
        THROW 50000, 'Kwota lub ilosc miejsc nie moze byc mniejsza lub rowna 0', 1;
    end
    if exists(select 1 from Studies where name = @name)
    begin
        THROW 50000, 'Podane studia juz istnieja', 1;
    end
    begin
        declare @studiesId int = dbo.nextStudiesId();
        declare @syllabusId int = dbo.nextSyllabusId();
        insert into Studies (studiesId, syllabusId, entryFee, meetFee, slotsLimit, name)
        values (@studiesId, @syllabusId, @entryFee, @meetFee, @slotsLimit, @name)
    end
end
GO

CREATE PROCEDURE AddStudiesAttendance
    @userId int,
...
BEGIN
    ...
    begin
        throw 50000, 'Użytkownik nie istnieje', 1;
    end

    if not exists (select * from StudiesMeetings where studiesMeetingId = @studiesMeetingId)
    begin
        throw 50000, 'Spotkanie nie istnieje', 1;
    end

    if (select date from StudiesMeetings where studiesMeetingId = @studiesMeetingId) > getdate()
    begin
        throw 50000, 'Nie można wpisać obecności na spotkanie, które się jeszcze nie odbyło', 1;
    end

    insert into StudiesAttendance (userId, studiesMeetingId) values
    (@userId, @studiesMeetingId)
...
CREATE PROCEDURE AddStudiesMeetingOffline
    @date date,
    @scheduleId int,
    @subjectId int,
    @place varchar(30),
...
BEGIN
    ...
    begin
        throw 50000, 'Niepoprawny przedmiot', 1;
    end

    if not exists (select * from StudiesSchedules where scheduleId = @scheduleId)
    begin
        throw 50000, 'Niepoprawny scheduleId', 1;
    end

    if @date < getdate()
    begin
        throw 50000, 'Nie można dodać spotkania, które się juz odbyło', 1;
    end
    declare @studiesMeetingId int = dbo.nextStudiesMeetingId();
    insert into StudiesMeetings (studiesMeetingId, date, scheduleId, subjectId) values
    (@studiesMeetingId, @date, @scheduleId, @subjectId)
    insert into OfflineStudiesMeetings (studiesMeetingId, place, room) values
    (@studiesMeetingId, @place, @room)
...
CREATE PROCEDURE AddStudiesMeetingOnline
    @date date,
    @scheduleId int,
    @subjectId int,
    @link varchar(50)
...
BEGIN
    ...
    insert into StudiesMeetings (studiesMeetingId, date, scheduleId, subjectId) values
    (@studiesMeetingId, @date, @scheduleId, @subjectId)
    insert into OnlineStudiesMeetings (studiesMeetingId, link) values
    (@studiesMeetingId, @link)
    print 'Poprawnie dodano spotkanie';
END
GO
...
BEGIN
    ...
    BEGIN
        THROW 50000, 'Niepoprawny studiesId', 1;
    END

    if exists (select * from StudiesSchedules where studiesId = @studiesId and semester = @semester)
    begin
        throw 50000, 'Terminarz już istnieje', 1;
    end

    if not exists (select * from Studies as s inner join Syllabuses as sb on
    s.syllabusId = sb.syllabusId where s.studiesId = @studiesId and sb.semester = @semester)
    begin
        throw 50000, 'Niepoprawny semestr', 1;
    end

    DECLARE @scheduleId int = dbo.nextStudiesScheduleId()
    INSERT INTO StudiesSchedules (scheduleId, studiesId, semester)
    VALUES (@scheduleId, @studiesId, @semester);
    print 'Poprawnie dodano terminarz'
END
GO
...
as
begin
    declare @subjectId int = dbo.nextSubjectId()
...
BEGIN
 IF NOT EXISTS (SELECT 1 FROM [Teachers] WHERE [teacherId] = @teacherId)
  BEGIN
     THROW 50000, 'Błąd: Podany użytkownik jest już nauczycielem.', 1;
  END
 IF NOT EXISTS(SELECT 1 FROM [AcademicsTitles] where [academicTitle] = @academicTitle)
  BEGIN
      THROW 50000, 'Błąd: Taki tytuł naukowy nie isnieje', 1;
  END
 ...
 PRINT 'Nauczyciel dodany poprawnie.';
END;
GO
...
BEGIN
    IF NOT EXISTS (SELECT *
        ...
        BEGIN
            ...
            RETURN
        END
    IF @type NOT IN ('course', 'webinar', 'studies')
        BEGIN
            RAISERROR ('Invalid type, %s', 16, 1, @type);
            RETURN
        END
    ...
    IF @educationFormId IS NULL
        BEGIN
            RAISERROR ('Education form with id %d does not exist', 16, 1, @specificId);
            RETURN
        END
    IF EXISTS (SELECT *
               FROM Cart
        ...
        BEGIN
            RAISERROR ('User already has the education with id: %d in cart', 16, 1, @educationFormId);
            RETURN
        END
    IF EXISTS (SELECT * FROM AssignedEducationForms WHERE userId = @userId AND educationFormId = @educationFormId)
        BEGIN
            RAISERROR ('User already has access to education form with id %d', 16, 1, @educationFormId);
            RETURN
        END
    IF @type = 'studies'
        BEGIN
            IF EXISTS(SELECT * FROM AwaitingStudents WHERE userId = @userId AND studiesId = @specificId)
                BEGIN
                    RAISERROR ('User is already awaiting for studies with id: %d', 16, 1, @specificId);
                    RETURN
                END
            IF EXISTS(SELECT * FROM Students WHERE userId = @userId AND studiesId = @specificId)
                BEGIN
                    RAISERROR ('User is already studying studies with id: %d', 16, 1, @specificId);
                    RETURN
                END
        END
...
AS
BEGIN
   IF NOT EXISTS (SELECT 1 FROM [Users] WHERE [userId] = @translatorID)
    BEGIN
       THROW 50000, 'Błąd: Podany użytkownik nie istnieje.', 1
       RETURN;
    END
   IF NOT EXISTS(SELECT 1 FROM [Translators] where [translatorId] = @translatorID)
    BEGIN
        THROW 50000, 'Błąd: Taki tłumacz już istnieje', 1
    END
...
AS
BEGIN
   IF @Miasto IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [Cities] WHERE [city] = @Miasto)
   BEGIN
       THROW 50000, 'Błąd: Podane miasto nie istnieje w tabeli Cities.', 1;
   END

   IF @Ulica IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [Streets] WHERE [street] = @Ulica)
   BEGIN
       THROW 50000, 'Błąd: Podana ulica nie istnieje w tabeli Streets.', 1;
   END

   IF @Wojewodztwo IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [States] WHERE [state] = @Wojewodztwo)
   BEGIN
       THROW 50000, 'Błąd: Podany stan nie istnieje w tabeli States.', 1;
   END
...
BEGIN
    DECLARE @type varchar(50) = (SELECT type
                                 FROM EducationForms
    ...
    IF EXISTS (SELECT *
        ...
        END
    PRINT 'Assigning education form to user'
...
BEGIN
    ...
    ELSE
        BEGIN
            DECLARE @webinarDetailsId int = dbo.nextWebinarDetailsId()
...
begin
with dane as (
       select userId, sum (entryFee + @meetingAmount * meetFee * S.semester) as do_zaplaty from Students S inner join Studies St on S.studiesId = St.studiesId
       group by userId
       union
...
begin
   if not exists(select * from Courses where courseId = @courseId)
   begin
       THROW 50000, 'Course not exists', 1;
   end
   delete from Courses
   where courseId = @courseId


   delete from EducationForms
   ...
   delete from EducationFormPrice
   where educationFormId = (select educationFormId from EducationForms
                           where specificId = @courseId)


   delete from StationaryModules
   where moduleId in (select moduleId from Modules where courseId = @courseId)


   delete from HybridModules
   where moduleId in (select moduleId from Modules where courseId = @courseId)


   delete from OnlineSyncModules
   where moduleId in (select moduleId from Modules where courseId = @courseId)


   delete from OnlineAsyncModules
   where moduleId in (select moduleId from Modules where courseId = @courseId)


   delete from Attendance
   where meetingId in (select meetingId from Meetings where meetingId in (select moduleId from Modules where courseId = @courseId))
   ...
   delete from AssignedEducationForms
   where educationFormId = (select educationFormId from EducationForms
                           where specificId = @courseId)

    delete from EducationFormPaymentsDue
    where educationFormId = (select educationFormId from EducationForms
                            where specificId = @courseId)
end
GO
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM Webinars WHERE webinarId = @webinarId)
        BEGIN
            THROW 50000, 'Webinar does not exists', 1;
        END
    DELETE
    FROM WebinarMeetings
    WHERE onlineMeetingId = (SELECT onlineMeetingId FROM Webinars WHERE webinarId = @webinarId)

    DECLARE @webinarDetailsId int = (SELECT webinarDetailsId FROM Webinars WHERE webinarId = @webinarId)
...
begin
    ...
    begin
        throw 50000, 'User not exists', 1
    end
    begin
        select name, surname, state, city, street, zip from Users
...
BEGIN
    IF NOT EXISTS (SELECT * FROM dbo.getCartForUser(@userId))
        BEGIN
...
begin
select C.title, 'kurs' as typ,  sum(amount) as przychod from PaymentsHistory inner join dbo.EducationForms EF on PaymentsHistory.payedFor = EF.educationFormId
inner join Courses C on courseId = specificId
group by C.title
union
select WD.title, 'webinar' as typ, sum(amount) as przychod from PaymentsHistory inner join dbo.EducationForms E on E.educationFormId = PaymentsHistory.payedFor
inner join Webinars W on webinarId = specificId
inner join WebinarDetails WD on W.webinarDetailsId = WD.webinarDetailsId
group by WD.title
union
select distinct Studies.name, 'studia', entryFee * (select count(*) from Students S2 where S2.studiesId = S.studiesId) + @meetingAmount * meetFee * (select count(*) from Students S2 where S2.studiesId = S.studiesId) from Students S
inner join Studies on S.studiesId = Studies.studiesId
end
GO

CREATE PROCEDURE InsertPaymentHistory
    @userId          int,
...
BEGIN
    ...
    if not exists (select * from EducationForms where educationFormId = @payedFor)
    begin
        throw 50000, 'Taka forma kształcenia nie istnieje', 1;
    end
    ...
    declare @paymentId int = dbo.nextPaymentId();
    INSERT INTO PaymentsHistory (paymentId, userId, paymentDate, payedFor, amount, paymentDetails)
